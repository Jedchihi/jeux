...
void movePerso(Personne *p, Background *bp)
{
  ...
  if(p->tabcollison[0]==0) {
    if (p->perso.positioninscrenn.x > 1200/2)
      bp->pos2.x += p->vitesse * p->dt;
    if (bp->pos2.x == 0 || bp->pos2.x > bp->bg->w-1200)
  ...
  case 1:
   if(p->tabcollison[1]==0)  {
    if (bp->pos2.x == 0 || bp->pos2.x >= bp->bg->w-1200)
      p->perso.positioninscrenn.x -= p->vitesse * p->dt;
    if (p->perso.positioninscrenn.x > 1200/2 && p->perso.positioninscrenn.x < 1200/2+10)
      bp->pos2.x -= p->vitesse * p->dt;
}
    break;
  }
  if (bp->pos2.x < 0)
    bp->pos2.x = 0;
  if (bp->pos2.x > bp->bg->w-1200)
    bp->pos2.x = bp->bg->w-1200;
};
